module.exports = app => {
  ...
  const smell2big = require('../middle/smell2big')
  const jwt = require('jsonwebtoken')
  const Admin = require('../model/Admin')
  const assert = require('http-assert')
  const mid = require('../middle/token')
  ...
  router.get('/', async(req,res) => {
    ...
    const item = await req.Model.find().setOptions(queryOptions).limit(200)
    res.send(item)
  })
  ...
  router.get('/:id',async(req,res) => {
    ...
    if(req.Model.modelName === 'Hero'){
      ...
      queryOptions.populate = 'item2'
    }
    const item1 = await req.Model.findById(req.params.id).setOptions(queryOptions)
    res.send(item1)
  })
  ...
  app.use('/wzry/api/common/:resource',smell2big(), router)

  const multer = require('multer')
  const upload = multer({dest:__dirname + '/../uploads'})
  app.post('/wzry/api/upload',upload.single('file') ,async(req,res) => {
    const file =await req.file
    file.url = `http://123.207.196.89:3002/uploads/${file.filename}`
    res.send(file)
  })

  app.post('/wzry/api/login',async(req,res) => {
  ...
  app.get('/wzry/api/news/init',async(req,res) => {
    ...
    await Article.deleteMany({}) //删除数据库
    await Article.insertMany(newsList) //插入数据
    res.send(newsList)
  })

  app.get('/wzry/api/news/list',async(req,res) => {
    const parents = await Category.findOne({
      name:'新闻分类'
    })
  ...
  app.get('/wzry/api/heroes/all',async(req,res) => {
    const parents = await Category.findOne({
      name:'英雄分类'
    })
    const cats = await Category.aggregate([
  ...
  app.get('/wzry/api/article/detail/:id',async(req,res) => {
    const article = await Article.findById(req.params.id).lean()
    article.relevant = await Article.find().where({
  ...
  //顺风出装接口
  app.get('/wzry/api/item1/:id',async(req,res) => {
    let queryOptions = {}
      queryOptions.populate = 'item1'
      // queryOptions.populate = 'partners.hero'
    const model = await Hero.findById(req.params.id).setOptions(queryOptions)
    res.send(model)
  })
  //搭档接口
  app.get('/wzry/api/friend/:id',async(req,res) => {
    let queryOptions = {}
      queryOptions.populate = 'partners.hero'
      const model = await Hero.findById(req.params.id).setOptions(queryOptions)
      res.send(model)
